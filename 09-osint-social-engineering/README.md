...
Filters results by file extension

filetype:pdf

intitle:

Searches for specific strings inside the page title

intitle:"index of"

inurl:

Searches for specific strings inside the URL structure

inurl:wp-content

intext:

Searches for specific keywords within the page body
...
Scenario C: Extracting Configuration Files and Hardcoded Credentials
Locating exposed environment files (.env), XML configurations, or database connection strings.

site:target.com filetype:env OR filetype:yaml OR filetype:conf "db_password"
...
Investigating overlay networks and performing network forensics to dissect anonymous routing behaviors.
  ...
  +-----------+      (Encrypted)      +------------+
  |  Client   | --------------------> | Entry Node | (Knows Client IP;
  +-----------+                       +------------+  Doesn't know target)
        |                                   |
        | (Double Encrypted)                | (Encrypted Layer Removed)
        v                                   v
  +-----------+                       +------------+
  |  Target   | <-------------------- | Exit Node  | (Knows Target;
  +-----------+      (Plaintext)      +------------+  Doesn't know Client)
                                            ^
                                            | (Encrypted Layer Removed)
                                      +------------+
                                      |Middle Node | (Relays traffic;
                                      +------------+  Zero visibility)
...
Urgency & Scarcity: Instilling artificial time constraints (e.g., "Your active directory access will expire in 45 minutes") to force rapid decision-making under stress.
...
🛠️ 4. Phishing Simulation: Social-Engineer Toolkit (SET)
...
Launch the tool inside Kali Linux under administrative privileges to allow network socket binding on restricted privileged ports (such as port 80):
...
Attacker IP Configuration: Input your local network interface IP (or external WAN IP if executing a WAN-scoped test). This is where the cloned website's login forms will send (POST) data.

Step 4: Clone the Target Web Interface

SET prompts for the URL to clone:

set:webattack> Enter the url to clone: https://portal.spartabank.local/login
...
Once the server initializes, the attacker sends the target link to the victim (usually masked using a typosquatted domain or an active ARP/DNS spoofing vector).

When the victim accesses the cloned page and submits their credentials, the console logs the transaction:
...
All tasks and simulated exercises were executed strictly within a localized sandbox environment for academic exploration, in compliance with the Portuguese Cybercrime Law framework (Lei n.º 109/2009)
